...
// MakeTagMap creates a map of tags with associated entries.
func MakeTagMap(entries []Entry) map[string][]int {
	tagIDs := make(map[string][]int)
	for _, e := range entries {
		for _, t := range e.Tags {
			tagIDs[t] = append(tagIDs[t], e.ID)
		}
	}
	return SortIDs(tagIDs)
}

// MakeTags creates the tag struct with counts
func MakeTags(tagIDs map[string][]int) []Tags {
	tagList := []Tags{}
...
func SortIDs(m map[string][]int) map[string][]int {
		for k, v := range m {
			sort.Ints(v)
			m[k] = v
	}
